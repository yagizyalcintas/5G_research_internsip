classdef wirelessNetworkSimulator < handle %% THIS IS IN NRGNB.M
    ...
    methods
        ...
        function addNodes(obj, nodes)
            ...
            obj.NodeNextInvokeTimes(1)=0.0205;
        end
        ...
        function run(obj, simulationDuration)
            ...
            while(obj.CurrentTime <= simulationDuration)
                ...
                for nodeIdx = 1:obj.NumNodes
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    % if obj.CurrentTime < 0.02
                    %     disp("obj.currtime inside loop: " + obj.CurrentTime);
                    %     obj.NodeNextInvokeTimes(1)=0.0205; 
                    % 
                    % elseif obj.CurrentTime == 0.02
                    %     obj.NodeNextInvokeTimes(1) = 0.0205;
                    %     obj.NodeNextInvokeTimes(2) = 0.0205;
                    %     obj.NodeNextInvokeTimes(3) = 0.0205;
                    %     obj.NodeNextInvokeTimes(4) = 0.0205;
                    % end
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    if obj.NodeNextInvokeTimes(nodeIdx) - obj.CurrentTime < 1e-9  %%IF WE CHANGE NODENEXTINVOKETIME IN AddNodes() IT SHOULD RUN LATER
                        ...
                        % if obj.NodeNextInvokeTimes(1) <= obj.CurrentTime
                        %     obj.NodeNextInvokeTimes(1) = obj.NodeNextInvokeTimes(2);
                        % end
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
                        %nextinvokeb4run = obj.NodeNextInvokeTimes(nodeIdx);
                        obj.NodeNextInvokeTimes(nodeIdx) = run(obj.Nodes{nodeIdx}, obj.CurrentTime);
                        % if nodeIdx == 1
                        %     obj.NodeNextInvokeTimes(nodeIdx) = obj.NodeNextInvokeTimes(2) + 0.0005;
                        % else
                        %     obj.NodeNextInvokeTimes(nodeIdx) = nextinvokeb4run + 0.0005;
                        % end
                ...
                disp("recentlyRanNodesIdx: " + recentlyRanNodesIdx);
                ...
                % if obj.CurrentTime < 0.04
                %     disp("obj.currtime after loop: " + obj.CurrentTime);
                %    obj.NodeNextInvokeTimes(1)=0.0405; 
                % 
                % elseif obj.CurrentTime == 0.04
                %     obj.NodeNextInvokeTimes(1) = obj.NodeNextInvokeTimes(2);
                % end
                % Calculate invoke time for next run
                disp("obj.nodenextinvoketimes3:");
                disp(obj.NodeNextInvokeTimes);
                nextRunTime = nextInvokeTime(obj);
                disp("next Run time:" + nextRunTime);
                %disp("object.currentTime: " + obj.CurrentTime);
    ...
    methods(Access = protected)
        ...
        function dt = nextInvokeTime(obj)
            ...
            if obj.NumNodes == 0
                ...
                disp("obj.NodeNextInvokeTimes in  dt=nextInvokeTime(obj): ");
                disp(obj.NodeNextInvokeTimes);
                disp("min(obj.NodeNextInvokeTimes)");
                disp(min(obj.NodeNextInvokeTimes));
                nextNodeDt = min(obj.NodeNextInvokeTimes);
                %disp("min(setdiff(obj.NodeNextInvokeTimes,min(obj.NodeNextInvokeTimes)))");
                %nextNodeDt = min(setdiff(obj.NodeNextInvokeTimes,min(obj.NodeNextInvokeTimes)));
                disp("nextNodeDt: ");
                disp(nextNodeDt);
            end
            if ~isempty(obj.ActionInvokeTimes)
                nextActionTimes = obj.ActionInvokeTimes(obj.ActionInvokeTimes ~= obj.CurrentTime);
                disp("nextActionTimes: ");
                disp(nextActionTimes);
                dt = min(nextActionTimes(1), nextNodeDt);
                disp("dt in if ~isempty(obj.ActionInvokeTimes): " + dt);
            else
                dt = nextNodeDt;
                disp("dt in else: " + dt);
            end
        ...
        function distributePackets(obj, recentlyRunNodesIdx)
            ...
            for txIdx = 1:numRecentlyRunNodes
                ...
                txData = pullTransmittedData(txNode); %% check here the format of txdata
                ...
                for pktIdx = 1:numel(txData)
                    ...
                    if txPacket.DirectToDestination == 0
                     ...
                     %%CHECK HERE
                                ...
                                obj.NodeNextInvokeTimes(rxIdx) = obj.CurrentTime; %% CHECK HERE
                            end
                        end
