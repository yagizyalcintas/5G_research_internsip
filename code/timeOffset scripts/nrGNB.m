classdef nrGNB < wirelessnetwork.internal.nrNode
    ...
    %   output, GNB is an array of gNB objects containing N gNBs.
    %   You can also supply multiple names for 'Name' argument corresponding to
    %   number of gNBs created. Multiple names must be supplied either as an
    ...
    %   node. Assuming 'N' nodes are created and 'M' names are supplied, if
    %   (M>N) then trailing (M-N) names are ignored, and if (N>M) then trailing
    %   (N-M) nodes are set to default names.
    %   You can set the "Position" and "Name" properties corresponding to the
    %   multiple gNBs simultaneously when you specify them as N-V arguments in
    %   the constructor. After the node creation, you can set the "Position"
    %   and "Name" property for only one gNB object at a time.
    %
    ...
    %   CarrierFrequency     - Frequency at which carrier served by gNB is operating
    ...
    %   DLULConfigTDD        - Downlink (DL) and uplink (UL) time division configuration
    %   NoiseFigure          - Noise figure in dB
    %   ReceiveGain          - Receiver antenna gain in dB
    %   NumHARQ              - Number of hybrid automatic repeat request (HARQ) processes for each user equipment (UE) which connects to the gNB
    %
    %   nrGNB properties (read-only):
    %
    %   ID                   - Node identifier
    %   ConnectedUEs         - Radio network temporary identifier (RNTI) of the connected UEs
    %
    %   Constant properties:
    %
    %   MCSTable             - MCS table used for DL and UL as per 3GPP TS 38.214 - Table 5.1.3.1-2
    %
    %   nrGNB methods:
    %
    ...
    %
    %   The scheduler at gNB assigns the resources in a round-robin (RR)
    %   manner. For DL channel measurements, the gNB transmits a full bandwidth
    %   channel state information reference signal (CSI-RS) which all the UEs
    %   in the cell use. For frequency division duplex (FDD) mode, the
    %   periodicity for CSI-RS transmission is 10 slots. For time division
    %   duplex (TDD) mode, the periodicity is 'M' slots. 'M' is the smallest
    %   integer which is greater than or equal to 10 and a multiple of the
    %   length of the DL-UL pattern (in slots). You can set different CSI
    %   reporting periodicity for the UEs using 'CSIReportPeriodicity'
    %   parameter of <a href="matlab:help('nrGNB.connectUE')">connectUE</a> method of this class. For UL channel measurements,
    %   gNB reserves one symbol for the sounding reference signal (SRS)
    %   periodically, across the entire bandwidth. For FDD, one SRS symbol is
    %   reserved every 5 slots. For TDD, one SRS symbol is reserved every 'N'
    %   slots. 'N' is the smallest integer which is greater than or equal to 5
    %   and a multiple of the length of the DL-UL pattern (in slots). The gNB
    %   configures the UEs to share the reservation of SRS bandwidth by
    %   differing the comb offset, cyclic shift, or transmission time. Comb
    %   size and maximum cyclic shift are both assumed as 4. In this case, gNB
    %   can configure up to 16 connected UEs to transmit SRS with periodicity
    %   as 'N' slots. If more than 16 UEs connect, the scheduler increases the
    %   SRS transmission periodicity for each UE to the next higher value (a
    %   multiple of 'N') to accommodate more UEs.
    %
    %   % Example 1:
    ...
    %               NumResourceBlocks=51, DLULConfigTDD=tddConfig)
    %
    %   % Example 3:
    ...
    properties (SetAccess = private)
        ...
        %   Specify the number of transmit antennas on gNB. The default
        %   value is 1.
        ...
        %   Specify the number of receive antennas on gNB. The default
        %   value is 1.
        ...
        %   connecting to the gNB operates in this SCS. The default value is 15e3.
    ...
    methods
        function obj = nrGNB(varargin)
            ...
            if nargin > 0
                ...
                positionIdx = find(strcmp([names{:}], 'Position'));
                numGNBs = 1;
                ...
                for i=1:2:numArgs-1
                    ...
                    switch (name)
                        case 'Position'
                            ...
                            for j = 1:numGNBs
                                obj(j).Position = value(j, :);
                            end
                        case 'Name'
                            % Set name for gNB(s). If name is not supplied
                            % for all gNBs then leave the trailing gNBs
                            % with default names
                            nameCount = min(numel(value), numGNBs);
                            for j=1:nameCount
                                obj(j).Name = value(j);
                            end
                        otherwise
                            % Make all the gNBs identical by setting same
        ...
        function connectUE(obj, UE, varargin)
            ...
            %
            %   RLCBearerConfig      - Object of type <a
            %                          href="matlab:help('nrRLCBearerConfig')">nrRLCBearerConfig</a>.
            %                          Use this option when full buffer is not enabled.
            %                          If you enable the full buffer on the DL or UL
            %                          direction, the object ignores this value. If you
            %                          do not enable the full buffer and you do not
            %                          specify this value, the object uses a default
            ...
            connectionConfigStruct = struct('RNTI', 0, 'GNBID', obj.ID, 'GNBName', ...
                ...
                'NumTransmitAntennas', 1, 'NumReceiveAntennas', 1,  'InitialMCSIndexDL', ...
                11, 'InitialMCSIndexUL', 11, 'InitialCQIDL', 0, 'InitialCQIUL', 0, ...
            ...
            for i=1:numUEs
                ...
                % Assign SRS configuration to connection and mark
                % the resource as occupied
                obj.SRSConfiguration(freeSRSIndex).NumSRSPorts = UE(i).NumTransmitAntennas;
                obj.SRSConfiguration(freeSRSIndex).NSRSID = obj.NCellID;
                connectionConfig.SRSConfiguration = obj.SRSConfiguration(freeSRSIndex);
                ...
                % Validate per UE connection information
        ...
        function configureScheduler(obj, varargin)
            ...
            %
            %   configureScheduler(OBJ, Name=Value) configures scheduler at gNB as per
            %   the configuration parameters specified in name-value arguments. You can
            %   set configuration parameters using name-value arguments in any order as
            %   (Name1=Value1,...,NameN=ValueN). When you do not specify a name-value
            %   argument corresponding to a configuration parameter, the method uses a
            %   default value for it. Use these name-value arguments to set scheduler
            %   parameters.
            %
            ...
            %                           NumResourseBlocks]. The default value is 6.
            ...
            coder.internal.errorIf(~isempty(obj.CurrentTime), 'nr5g:nrNode:NotSupportedOperation', 'configureScheduler');
            coder.internal.errorIf(~isempty(obj.ConnectedUEs), 'nr5g:nrGNB:ConfigSchedulerAfterConnectUE');

            schedulerInfo = struct(Scheduler='RoundRobin', ResourceAllocationType=1, ...
            ...
            for idx=1:2:nargin-1
                ...
                if name == "MUMIMOConfigDL"
                    schedulerInfo.MUMIMOConfigDL = nr5g.internal.nrNodeValidation.validateConfigureSchedulerMUMIMOInputs(obj, varargin{idx+1}, mumimoConfigDL); %%CHECK HERE
                else
            ...
            if ~isempty(schedulerInfo.MUMIMOConfigDL)
                obj.CSIReportType = 2; % Type II feedback for MU-MIMO transmission
            end
            ...
            for idx=1:numel(schedulerParam)
                schedulerInfo.(char(schedulerParam{idx})) = obj.(char(schedulerParam{idx}));
            end
            % Convert the SCS value from Hz to kHz
            schedulerInfo.SubcarrierSpacing = obj.SubcarrierSpacing/1e3;
            % Create round robin scheduler object and add it to MAC
            scheduler = nr5g.internal.nrSchedulerRoundRobin(schedulerInfo);
    ...
    methods(Access = protected)
        function setLayerInterfaces(obj)
            ...
            % (2) Sending the measured UL channel quality to MAC
             ...
             %%CHECK HERE: check the callback fucntion for adding a delay
            ...
            registerInBandTxFcn(phyEntity, @obj.addToTxBuffer);
        end
        ...
        function [srsReservedResource, srsConfiguration] = createSRSConfiguration(obj, varargin)
            ...
            %%CHECK HERE: WE COULD CREATE AN OFFSET GIVING MORE RESOURCES TO
            %%SRS
            minSRSPeriodicity = 5;
            if strcmp(obj.DuplexMode, "FDD") % FDD
                ...
                % Set SRS resource periodicity as mimimum value such that it is at least 5
